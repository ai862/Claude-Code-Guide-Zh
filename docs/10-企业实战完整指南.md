...
> - **难度等级**：⭐⭐⭐⭐⭐ 高级
> - **更新日期**：2026年1月19日
> - **适用版本**：Claude Code 2.1.12+（验证于2026-01-19）
...
| **Trivy** | - | 容器镜像安全扫描工具 |
| **Forked Context** | - | Skills的独立上下文（2.1.0+） |
| **Hot Reload** | - | Skills热重载（2.1.0+） |
...
│   ├── sidebar.json             # 侧边栏配置（2.1.0+支持右侧边栏）
│   └── snippets/                # 代码片段
│       ├── python.json
│       └── javascript.json
├── cursor/                      # Cursor配置
...
│   │   ├── refactor.md          # 重构提示词
│   │   └── testing.md           # 测试生成提示词
│   ├── language.json            # 语言配置（2.1.0+支持多语言响应）
│   ├── skills/                  # 团队技能包（2.1.0+支持/调用和hot reload）
│   │   └── team-standards/
...
**2.1.0+新特性说明**：
- Hooks现在可以直接添加到skills的frontmatter中
- 无需单独配置Git hooks目录
...
# 2.1.0+新特性：在skill frontmatter中直接定义hooks
hooks:
  - event: pre-commit
...
**文档结束** | 最后更新：2026年1月19日 | 版本：V1.1.0
